...
function smarty_function_html_select_date($params, $template)
{
    ...
    if ($display_months) {
        ...
        for ($i = 1; $i <= 12; $i++) {
            $_val = sprintf('%02d', $i);
            $_text = isset($month_names) ? smarty_function_escape_special_chars($month_names[$i]) : ($month_format == "%m" ? $_val : gmstrftime($month_format, $_month_timestamps[$i]));
            $_value = $month_value_format == "%m" ? $_val : gmstrftime($month_value_format, $_month_timestamps[$i]);
